...
const toggleLang = () => {
  ...
  new Audio(BASE_URL + "sounds/click.mp3").play().catch(() => {});
};

// 按钮跳转逻辑
const handleNav = (path) => {
  const audio = new Audio(BASE_URL + "sounds/click.mp3");
  audio.volume = 0.5;
  audio.play().catch(() => {});
...
<template>
  <div class="mc-container">
    ...
    <nav class="menu-list">
      ...
      <div class="mc-btn" @click="handleNav('/mc/messages')">
        {{ t("mc.menu_board") }}
      </div>
    ...
    <footer class="version-text">Minecraft 1.20.1 (Modded by biscuit)</footer>
  </div>
</template>
...
.logo {
  margin: 0 15px;
  filter: drop-shadow(4px 4px 0 #3f3f3f);
}
.subtitle {
  color: #ffff55;
  font-size: 24px;
